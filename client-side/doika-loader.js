...
(function() {
	...
	// Dock donate module banner to top of the page and slide down fixed
	// elements(like top menus)
	...
	function dockBannerToTop() {
		if (!window.doika.bannerDocked && window.doika.banner_visibility) {
			...
			for (var i = 0; i < elems.length; i++) {
				var elementComputedStyle = window.getComputedStyle(elems[i],
						null);
				var elementPosition = elementComputedStyle
						.getPropertyValue('position');
				var elementTopPosition = elementComputedStyle
						.getPropertyValue('top');

				if ((elementPosition == 'fixed')
						&& (parseInt(elementTopPosition) <= 59)
						&& (elementTopPosition)) {
			...
			donateHeader.innerHTML = '<p class="donateHeader__title">' + title
					+ '</p>' + '<p class="donateHeader__goal">' + goal + '</p>'
			...
			var moduleDOMElement = document
					.querySelector("#module-donate-wrapper");
		...
		if (window.doika.banner_visibility) {
			...
			if ((rect.bottom - delta) > 0
				...
				for ( var i in transformedElements) {
					transformedElements[i].classList
							.remove("donateHeader__transform");
				}
			} else {
	...
	function init() {
		...
		document.querySelector(".b-popup-close").addEventListener("click",
				function() {
					PopUpHide(popup);
				});
		...
		document.querySelector(".b-popup-content").addEventListener("click",
				function(e) {
					e.stopPropagation();
				});
	...
	function getUrlParameter(sParam) {
		var sPageURL = decodeURIComponent(window.location.search.substring(1)
				.replace(/\+/g, '%20')), sURLVariables = sPageURL.split('&'), sParameterName, i;
	...
	function loadDonateModule() {
		...
		case 'fail':
			...
			window.doika.statusMessage = getUrlParameter("statusMessage");
			wrapper.innerHTML = '<iframe id="module-donate" src="/client-side/module-donate-payment.html" frameborder="0" scrolling=no height="0" width="100%"></iframe>';
			break;
		default:
					...
					.addEventListener(
							...
							function(e) {
								...
								case 'updateIframeHeight':
									var donateModule = document
											.getElementById('module-donate');
									donateModule.style.height = donateModule.contentWindow.document.body.scrollHeight
											+ 'px';
									wrapper.style.height = donateModule.contentWindow.document.body.scrollHeight
											+ 'px';
									break;
								case 'doikaSubmit':
									wrapper.innerHTML = '<iframe id="module-donate" src="/client-side/module-donate-payment.html" frameborder="0" scrolling=no height="0" width="100%"></iframe>';
								...
								case 'dockHeader':
									if (!document
											.querySelector(".donateHeader")
											&& window.doika.banner_visibility) {
										dockBannerToTop();
										var moduleDOMElement = document
												.querySelector("#module-donate-wrapper");
										var banner = document
	...
	function addPopUpToDOM() {
		...
		div.innerHTML = '<div class="b-popup-content">'
				...
				+ '<p>Аплата банкаўскімі картамі ажыццяўляецца праз ЗАТ <a href="https://alfa-biz.by/"><span style="color:#DC143C">«Альфа-Банк»</span></a>.</p>'
				+ '<p>Да аплаты прымаюцца карты міжнародных плацёжных сістэм VISA, MasterCard. '
				+ 'Бяспека здзяйснення плацяжу забяспечваецца сучаснымі метадамі праверкі, шыфравання і перадачы дадзеных па закрытых каналах сувязі. '
				+ 'Увод дадзеных карткі ажыццяўляецца на абароненай аўтарызацыйнай старонцы банка. Для аплаты неабходна ўвесці рэквізіты карткі: нумар, імя трымальніка, тэрмін дзеяння і трохзначны код бяспекі. Трохзначны код бяспекі (CVV2 для VISA, CVC2 для MasterCard) - гэта тры лічбы, якія знаходзяцца на адваротным баку карткі. '
				+ 'Калі картка падтрымлівае тэхналогію 3DSecure, для ўводу кода бяспекі Вы будзеце перанакіраваны на старонку банка, які выпусціў картку. '
				+ 'Персанальная інфармацыя, якая прадастаўляецца Вамі, (напрыклад: імя, адрас, тэлефон, e-mail, нумар банкаўскай карты і іншае) з\'яўляецца канфідэнцыйнай і не падлягае разгалашэнню. Дадзеныя карткі перадаюцца толькі ў зашыфраваным выглядзе і не захоўваюцца на дадзеным інтэрнэт-рэсурсе.</p>'
				+ '<p>Зварот грашовых сум, калі вы ўжо здзейснілі ахвяраванне, не ажыццяўляецца.</p>'
				+ '</div>';

		/*
		 * <!--<div class="b-popup-content-img"> <img
		 * id="b-popup-content-img-visa"
		 * src="https://lh4.googleusercontent.com/9EEq8ZYJPpco2xK00gYJFFUx_Stj37Nb5wLQanbnBU5ELcPdOan1UAy_jeUqGNFdCAoWC0PT_5AXfjwhZcPrBR1JXsrf9XGcv58mR-ktyN0vPN77gRdOaXXg1i-oCKX-CzkKK4vl=s800">
		 * <img id="b-popup-content-img-belcard"
		 * src="https://lh6.googleusercontent.com/Bt1eFBKo9amovmut4a08H93W1863_8c-a24F5S-vvXiQkVTbk44B5x9O-k4bIz6S93spBuuUyAD8dVr4l7Hk-KCX6crgyMo8tN5NCra707A1sAlmzmVInE_NJKgWrf3rplqdIshN">
		 * <img id="b-popup-content-img-bepaid" src="img/bepaid.png"> <img
		 * id="b-popup-content-img-mtbank" src="img/mtbank.png"> </div>-->
		 */
